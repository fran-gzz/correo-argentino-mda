...
| 1.2 | `typescript` y `@astrojs/check` en `dependencies` | 🟡 | Movidos a `devDependencies` (parcial: `@astrojs/check` aún en ambas secciones) | Sesión anterior |
...
| 1.6 | `NODE_ENV="development"` hardcodeado en esbuild define | 🟡 | Reemplazado por `process.env.NODE_ENV ?? "development"` | `cc946e6` |
...
| `fieldset-legend uppercase tracking-wide` (+ `mb-2`) | 0 (✅ resuelto vía FormLegend) | Admin CRUD |
